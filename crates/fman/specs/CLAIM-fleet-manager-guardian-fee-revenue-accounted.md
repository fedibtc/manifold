...
votes for a guardian-fee policy that the deployed payer will reject, and it
...
2. every whole-metadata object this daemon submits either contains no
   ...
   canonical payer-parseable recipient vector with FI at weight four, every
   guardian at weight one, and the Guardian Verification Fee at weight one,
   derived from the endpoint-verified DKG transcript, consensus seat directory,
...
or Guardian Verification Fee account, a guardian colliding with the Guardian
Verification Fee account, or a purported combined FI-and-guardian weight-five
entry fails closed. Setup payments and every fee
...
- **A3 payer contract:** the deployed payer reads
