...
- `abandon_gateway_item` — the gateway counterpart, and it wants the same care
  for the same reason. It is reached from a gateway item escalated to
  `action_required` after `funding_policy.gateway_claim_review_after_secs`, so
  the screen that shows that escalation is where it belongs
